...
struct DrawBox {
    ...
    int classId;
    float probability;
    QString plateText;  // 車牌文字（從 sKeypoints[i].nZ 解析，若無則為空字串）
};
...
public:
    ...
    // DWORD flag = 1 | 4; // 1: TRACKING, 4: FEATURE_VECTOR (用來取得車牌文字)
    DWORD flag = 1; // 1: TRACKING
    std::mutex mtx;
    ...
    std::vector<DrawBox> draw_boxes[MAX_DEVICE];
    std::mutex draw_mtx;

    OverlayWidget *overlayWidget = nullptr;
    bool m_bDrawBoxes = true;
    QCheckBox *m_pChkDrawBoxes = nullptr;
