...
void MainWindow::onChannelCountChanged(int count)
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (!itemUrl || itemUrl->text().isEmpty()) {
            QString defaultUrl = QString("rtsp://root:root@192.168.191.23:554/session0.mpg");
...
void MainWindow::yolo_start()
{
    ...
    if (res != QCAP_RS_SUCCESSFUL || handle == nullptr) {
        qCritical() << "QDEEP plate create failed, nMaxBatch=" << nMaxBatch << "qres=" << res;
        handle = nullptr;
        return;
    }
    ...
    if (res != QCAP_RS_SUCCESSFUL) {
        qCritical() << "QDEEP plate start failed, qres=" << res;
        QDEEP_API::QDEEP_DESTROY_OBJECT_DETECT(handle);
        handle = nullptr;
        return;
    }
    ...
    if (res != QCAP_RS_SUCCESSFUL) {
        qCritical() << "QDEEP plate reserved-status failed, qres=" << res;
        QDEEP_API::QDEEP_STOP_OBJECT_DETECT(handle);
        QDEEP_API::QDEEP_DESTROY_OBJECT_DETECT(handle);
        handle = nullptr;
        return;
...
void MainWindow::ai_inference_thread()
{
    ...
    while (ai_running) {
        ...
        const double api_ms = std::chrono::duration<double, std::milli>(
        ...
        if (apiRes != QCAP_RS_SUCCESSFUL) {
            qCritical() << "QDEEP plate batch submission failed, batch=" << batch_size
                        << "qres=" << apiRes;
        }
